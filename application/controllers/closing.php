...
class Closing extends CI_Controller
{
	...
	{
		$data['page_title'] = lang('closings');
		$data['action'] = 'closing/add_submit';
		$data = add_fields($data, 'closing');

		$data['locations'] = location_options($this->locationModel->get_all_locations());
		
		$this->load->view('templates/header', $data);
		$this->load->view('closing_edit_view', $data);
		$this->load->view('templates/footer');
	}

	/** Submits the addition of a closing */
	...
	/** Page to edit a closing */
	public function edit($closing_id)
	{
		$closing = $this->closingModel->get_closing_by_id($closing_id);

		$data['page_title'] = lang('closings');
		$data['action'] = 'closing/edit_submit/' . $closing_id;
		$data = add_fields($data, 'closing', $closing);

		$data['locations'] = location_options($this->locationModel->get_all_locations());

		$data['lockdown'] = !$closing->location_id;
		$data['from'] = output_datetime($closing->from, TRUE);
		$data['end'] = output_datetime($closing->to, TRUE);
		$data['all_day'] = FALSE;
		$data['from'] = output_date($closing->from, TRUE);
		$data['current_location_id'] = $closing->location_id ? array($closing->location_id) : array(); 
	...
	/** Submits the edit of a closing */
	public function edit_submit($closing_id)
	{
		$c = $this->closingModel->get_closing_by_id($closing_id);
